...
const luxuryCampaign=document.querySelector('.dark-couture-card');
if(luxuryCampaign){
  const project=luxuryCampaign.closest('.project');
  if(project){
    const label=luxuryCampaign.querySelector('small');
    const title=luxuryCampaign.closest('.project').querySelector('h3');
    ...
    if(link)link.textContent='Enter the journey ↗';
  }
}